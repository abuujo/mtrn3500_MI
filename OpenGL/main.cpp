...
double northing_values[2000];
double easting_values[2000];
int gps_index = 0;
...
void drawLaser(laser_data* LSObjPtr) {
	glPushMatrix();
	...
	glBegin(GL_LINES);
	// Put laser dots here
	for (int i = 0; i < LSObjPtr->num_data; i++) {
		addLine((LSObjPtr->x_coords[i]*0.001), (LSObjPtr->y_coords[i]*0.001));
	}
	glEnd();
	glPopMatrix();
}
...
void drawGPSValues() {
	...
	for (int i = 0; i < gps_index; i++) {
		addLine(northing_values[i],easting_values[i]);
	}
	glEnd();
	glPopMatrix();
...
void display() {
	...
	SMObject GPSObj(_TEXT("GPS"), sizeof(GPS));			// GPS
	GPSObj.SMCreate();
	GPSObj.SMAccess();
	...
	if (gps_index == 0) {
		northing_values[gps_index] = GPSObjPtr->northing;
		easting_values[gps_index] = GPSObjPtr->easting;
		gps_index++;
	}
	else if (
		!((northing_values[gps_index-1] == GPSObjPtr->northing) && (easting_values[gps_index-1] == GPSObjPtr->easting)) 
		&& gps_index < 2000) {
		northing_values[gps_index] = GPSObjPtr->northing;
		easting_values[gps_index] = GPSObjPtr->easting;
		gps_index++;
